			...
			<div className={`mx-auto max-w-7xl ${isEmbedded ? "p-2" : "px-4 py-8 sm:px-6 sm:py-10 lg:px-8"}`}>
				...
				<div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
					...
					<aside className="space-y-4">
						<section className="rounded-2xl border border-blue-500/20 bg-blue-50/60 p-5 dark:bg-[#0D1524]">
							<div className="mb-4 flex items-center gap-3">
								...
								<h2 className="text-sm font-bold text-slate-900 dark:text-white">What happens next?</h2>
							</div>
							<ol className="space-y-4 text-sm text-slate-600 dark:text-slate-400">
								<li className="flex gap-3">
									<span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-blue-500/15 text-xs font-bold text-blue-600 dark:text-blue-400">1</span>
									<span>Your issue is saved to your personal reported issues log with date & priority.</span>
								</li>
								<li className="flex gap-3">
									<span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-blue-500/15 text-xs font-bold text-blue-600 dark:text-blue-400">2</span>
									<span>Our team reviews and verifies the issue details.</span>
								</li>
								<li className="flex gap-3">
									<span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-blue-500/15 text-xs font-bold text-blue-600 dark:text-blue-400">3</span>
									<span>It is routed to the right community team for immediate action.</span>
								</li>
							</ol>
						</section>
						<section className="rounded-2xl border border-amber-500/30 bg-amber-50 p-5 dark:bg-amber-500/5">
							<div className="flex gap-3">
								<AlertTriangle size={18} className="shrink-0 text-amber-600 dark:text-amber-400" />
								<div>
									<h2 className="text-sm font-bold text-amber-800 dark:text-amber-300">Emergency?</h2>
									<p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-400">For immediate danger or a crime in progress, use the emergency service instead.</p>
								</div>
							</div>
						</section>
					</aside>
